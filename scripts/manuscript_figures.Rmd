...
all.params.data <- master.dat[, -c(1:7, 9:11, 14)] 

#rename various columns in data frame
names(all.params.data)[1] <- "Kelp Cover (%)"
names(all.params.data)[2] <- "Fish Biomass (g/cm2)"
names(all.params.data)[3] <- "Rugosity"
names(all.params.data)[4] <- "Kelp Biomass (g/cm2)"


#reorder columns to show biomass by eaxchother
all.params.data <- all.params.data[, c(3, 1, 2, 4)]
...
cormat <- round(cor(all.params.data, method = "spearman"), 4)
head(cormat)
...
## Calculate p-values
p_values <- cor.mtest(all.params.data, method = "spearman")$p
...
names(all.params.data) <- c("Fish_Biomass", "Rugosity", "Kelp_Cover", "Kelp_Biomass", "predictions")

# Fit a multiple regression model
fit4 <- lm(Fish_Biomass ~ Rugosity * Kelp_Biomass * Kelp_Cover, data = all.params.data)

# Diagnostic plots for the regression model
par(mfrow=c(2,2))
plot(fit4)

# Use ggPredict to visualize the multiple regression plot interactively
interactive_plot <- ggPredict(fit4, interactive = TRUE) 

# Save the interactive plot as an HTML file
htmlwidgets::saveWidget(interactive_plot, file = "output/figures/interactive_regression_plot.html")
...
(Intercept)                                      -199.41221   96.09880  -2.075   0.0473 *
Rugosity                                           98.82780   36.09379   2.738   0.0106 *
`Kelp Biomass (g/cm2)`                              0.41643    0.18683   2.229   0.0340 *
`Kelp Cover (%)`                                 -765.82428  612.85640  -1.250   0.2218  
Rugosity:`Kelp Biomass (g/cm2)`                    -0.12496    0.05199  -2.403   0.0231 *
Rugosity:`Kelp Cover (%)`                         375.38109  244.24549   1.537   0.1355  
`Kelp Biomass (g/cm2)`:`Kelp Cover (%)`            -0.24978    0.91866  -0.272   0.7877  
Rugosity:`Kelp Biomass (g/cm2)`:`Kelp Cover (%)`   -0.05647    0.24991  -0.226   0.8229  

# Try another type of multiple regession plot

```{r}
#fit multiple linear regression model
model2 <- lm(Fish_Biomass ~ Rugosity * Kelp_Biomass * Kelp_Cover, data = all.params.data)

#view results of model
summary_text <- capture.output(summary(model2))
writeLines(summary_text, "output/figures/summary_model.txt")

# Produce added variable plots
av_plots <- avPlots(model2)
