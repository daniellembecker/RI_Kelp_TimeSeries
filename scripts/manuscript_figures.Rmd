...
production.plot <- ggplot(PB_filtered, aes(x = YEAR, y = PB.ratio, fill = SP_CODE)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 18),  # Rotate x-axis labels if needed
        ...
        axis.text.y = element_text(size = 18),
        legend.text = element_text(size = 16),
        legend.title = element_text(size = 18)) + # Adjust y-axis title position
  scale_y_continuous(limits = c(0, 4), expand = c(0, 0));production.plot  # Set y-axis limits and remove expansion
...
# We add a small constant (1e-6) to avoid issues with log(0)
HPI_calculated <- HPI_calculated %>%
  mutate(log_HPI = log(HPI + 1.1))

# Now plot with the log-transformed HPI
species.hpi.plot <- ggplot(HPI_calculated, aes(x = SITE, y = log_HPI, fill = factor(YEAR))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Site", 
  ...
  coord_cartesian(clip = "off"); species.hpi.plot
...
```{r}
# Combine the plots with patchwork for all years of kelp and rate of change
production.hpi <- production.plot | species_hpi.plot +
  plot_annotation(tag_levels = 'A') + 
