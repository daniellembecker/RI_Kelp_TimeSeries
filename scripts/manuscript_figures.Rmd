...
PB_filtered$biomass <- with(PB, coef.a * (actual_size)^coef.b)

# Divide biomass by 80 m to get g/m2
#calculate biomass/m2
PB$biomass.g.m2 <- PB$biomass / 80

# Constants from the P/B ratio equation
coefficient <- 2.64
exponent <- -0.35

# Calculate P/B ratio, the predictive equation is P/B (per year) = 2.64*Wmat^-035 , where Wmat is weight-at-maturity (Randall and Minns 2000)
PB$P_B_ratio <- coefficient * PB$biomass.g.m2^exponent
...
PB$HPI <- PB$biomass.g.m2 * PB$P_B_ratio
...
# Make faceted line plot of HPI index over the years
hpi.plot < - ggplot(hpi_summary, aes(x = interaction(TRANSECT, SITE, sep = " - "), y = mean_HPI, fill = as.factor(YEAR))) +
  geom_bar(stat = "identity", position = "dodge", color = "black") +
  labs(x = "Transect - Site", y = "Mean Habitat Productivity Index (HPI)", fill = "Year") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1));hpi.plot
...
custom_shape_values <- c(16, 17, 18, 19, 20, 21, 22, 23)
