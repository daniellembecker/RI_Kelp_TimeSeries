...
production_plot <- ggplot(pb_summary, aes(x = YEAR, y = mean_production, color = TREATMENT, group = TREATMENT)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 18),  # Rotate x-axis labels if needed
        axis.title.x = element_text(vjust = -0.5, size = 18),  # Adjust x-axis title position
...
# Save the plot
ggsave("output/figures/production.habitat.png", production_plot_A, width = 20, height = 10, units = "in", dpi = 300)
...
production_diff_plot <- ggplot(merged_data, aes(x = YEAR, y = production_difference, group = TREATMENT_kelp)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 18),  # Rotate x-axis labels if needed
        axis.title.x = element_text(vjust = -0.5, size = 18),  # Adjust x-axis title position
        axis.title.y = element_text(vjust = 1.5, size = 18),  # Adjust y-axis title position
        axis.text.y = element_text(size = 18),  # Adjust y-axis text size
        legend.text = element_text(size = 16),  # Legend text size
        legend.title = element_text(size = 18));production_diff_plot
...
production_diff_plot_annot <- production_diff_plot +
  geom_text(aes(label = sprintf("%.2f", production_difference), y = production_difference),
            color = "black", size = 4, vjust = 2.42, hjust = 1.06, check_overlap = TRUE);production_diff_plot_annot

# Save the plot
ggsave("output/figures/kelp.fish.production.png", production_diff_plot, width = 20, height = 10, units = "in", dpi = 300)
...
```{r}
# Combine the plots with patchwork
prod_combined_plot <- (production_plot | production_diff_plot) +
  plot_annotation(tag_levels = 'A') + 
  theme(plot.tag = element_text(size = 20, face = "bold"));prod_combined_plot

# Save the combined plot
ggsave("output/figures/fish_production.png", prod_combined_plot, width = 12, height = 8, units = "in", dpi = 300)
