...
writeLines(summary_text1, "output/figures/summary_original_model.txt")

# Fit a multiple regression model with site and year as interactions
fit2 <- lm(Fish_Biomass ~ Rugosity * Kelp_Biomass * Kelp_Cover * SITE * YEAR, data = all.params.data)
...
writeLines(summary_text2, "output/figures/summary_model_site_year_fixed.txt")
...
fit3 <- lm(Fish_Biomass ~ SITE*YEAR, data = all.params.data)

#view results of model
summary_text3 <- capture.output(summary(fit3))
writeLines(summary_text3, "output/figures/summary_model_site_year_fixed_fishbiomass.txt")
...
writeLines(summary_text4, "output/figures/summary_model_site_year_random.txt")
...
writeLines(summary_text5, "output/figures/summary_model_site_random.txt")

# site and year no significant main effects

# run with just year as random effect
# Fit the mixed-effects model
fit6 <- lmer(log(Fish_Biomass) ~ log(Rugosity) * log(Kelp_Biomass) * log(Kelp_Cover) + 
              (1 | YEAR), data = all.params.data)

#view results of model
summary_text6 <- capture.output(summary(fit6))
writeLines(summary_text6, "output/figures/summary_model_year_random.txt")
