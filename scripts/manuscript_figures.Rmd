...
```{r}
# Calculate mean and standard error of temperature by month and site
...
wide_data <- wide_data %>%
  mutate(across(crustacean:molluscs, as.numeric))

# Calculate Bray-Curtis dissimilarity
m_com <- vegdist(wide_data[, -c(4:6)], method = "bray")
...
plot_data <- data.frame(
  NMDS1 = nmds$points[, 1],  # NMDS dimension 1
  NMDS2 = nmds$points[, 2],  # NMDS dimension 2
  type = invert.count$type,  # Type of invertebrate
  abundance = invert.count$abundance  # Abundance of invertebrate
)

# Plot nMDS with type labels and scaled points by abundance
ggplot(plot_data, aes(x = NMDS1, y = NMDS2, color = type, size = abundance)) +
  geom_point(alpha = 0.8) +
  scale_size_continuous(range = c(2, 8)) +  # Adjust point size range as needed
  labs(title = "nMDS Plot of Invertebrate Types by Abundance",
       x = "NMDS 1", y = "NMDS 2") +
  theme_minimal() +
  theme(legend.position = "right") +
  geom_text(aes(label = type), nudge_x = 0.05, nudge_y = 0.05, size = 3)  # Add type labels
```
