...
# Convert sites dataframe to an sf object with CRS 4326
sites_sf <- st_as_sf(sites, coords = c("Longitude", "Latitude"), crs = 4326)

# Get Rhode Island state boundaries and set CRS to 4326
ri_state <- st_as_sf(map("state", plot = FALSE, fill = TRUE)) %>%
  filter(ID == "rhode island")

# Define your rectangle sizes (adjust as needed)
rect_size1 <- 0.01 # Smaller rectangle
rect_size2 <- 0.02 # Larger rectangle
...
# Apply the create_rectangle function to each site for both rectangle sizes
site_rectangles <- lapply(st_geometry(sites_sf), function(site) {
  rectangle1 <- create_rectangle(site, rect_size1)
  rectangle2 <- create_rectangle(site, rect_size2)
  list(rectangle1, rectangle2)
})

# Combine all rectangles into one sf object
site_rectangles <- do.call(rbind, unlist(site_rectangles, recursive = FALSE))

# Set CRS for combined rectangles
st_crs(site_rectangles) <- st_crs(sites_sf)

# Plot
panelA <- ggplot() +
  geom_sf(data = ri_state, fill = "lightgray") + # State boundaries
  geom_sf(data = site_rectangles, color = "red2", fill = NA, size = 1) + # Add rectangles around sites
