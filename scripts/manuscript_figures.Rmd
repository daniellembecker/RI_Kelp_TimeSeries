...
# Calculate total weight for each fish species
...
# Calculate production for each species in each transect
production_data <- PB_filtered %>%
  group_by(TRANSECT, SP_CODE, YEAR) %>%
  summarise(
    Total_Biomass = sum(biomass_per_m2, na.rm = TRUE),  # Sum of biomass for the species in the transect
    Total_Production = sum(biomass_per_m2 * PB.ratio, na.rm = TRUE)  # Total production for the species in the transect
  )

# Determine the maximum and minimum values for Total_Production
max_prod <- max(production_data$Total_Production)
min_prod <- min(production_data$Total_Production)

# Plotting using ggplot2
production.plot <- ggplot(production_data, aes(x = YEAR, y = Total_Production, fill = SP_CODE)) +
  ...
  scale_y_continuous(limits = c(0, 650), expand = c(0, 0));production.plot  # Set y-axis limits and remove expansion
...
# Calculate average biomass per SP_CODE
average_biomass_df <- PB_filtered %>%
  group_by(SP_CODE, PB.ratio, YEAR, TRANSECT, SITE) %>%
  summarise(average_biomass = mean(biomass_per_m2, na.rm = TRUE))

# Calculate Habitat Productivity Index (HPI)
average_biomass_df$HPI <- average_biomass_df$average_biomass * average_biomass_df$PB.ratio

# Aggregate by Transect, Site, Year and calculate mean HPI
hpi_summary <- average_biomass_df %>%
  group_by(SITE, YEAR) %>%
  summarise(mean_HPI = mean(HPI, na.rm = TRUE))

# Make faceted line plot of HPI index over the years as a area plot
hpi.plot <- ggplot(hpi_summary, aes(x = YEAR, y = mean_HPI, fill = SITE)) +
  geom_area(position = "stack") +
  labs(x = "Year",
       y = "Mean Habitat productivity index",
       fill = "Site") +
  theme_minimal(); hpi.plot
