...
Production to biomass ratios (P:B) and habitat productivity index for each fish species
```{r}
...
# Calculate production by multiplying biomass_per_m2 by PB raatio
PB_filtered$Production <- PB_filtered$PB.ratio * PB_filtered$biomass_per_m2

# make commmon name column
PB_filtered <- PB_filtered %>%
...
production_plot <- ggplot(PB_filtered, aes(x = YEAR, y = Production, fill = Species)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Year",  # X-axis label
       y = expression("Fish Production" ~ (g ~ m^-2 ~ yr^-1)),  # Y-axis label
       fill = "Species") +  # This should now correctly set the legend title
  scale_fill_viridis_d() +  # Use viridis colorblind-friendly palette
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 18),  # Rotate x-axis labels if needed
        ...
        legend.text = element_text(size = 16),
        legend.title = element_text(size = 18)) +
  scale_y_continuous(limits = c(0, 4), expand = c(0, 0));production_plot  # Additional attempt to change legend title  # Use viridis colorblind-friendly palette
...
#my_palette <- brewer.pal(n = 6, name = "Paired")  # Adjust 'n' as per the number of years in your data
