...
production_plot <- ggplot(PB_filtered, aes(x = YEAR, y = Production, fill = Species)) +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 12),  # Rotate x-axis labels if needed
        axis.title.x = element_text(vjust = -0.5, size = 14),  # Adjust x-axis title position
...
HPI_calculated <- HPI_calculated %>%
  mutate(log_HPI = log(HPI + 1.1))

# Define the color palette
my_palette <- brewer.pal(n = 6, name = "Paired")  # Adjust 'n' as per the number of years in your data

# Plotting code
hpi.plot <- ggplot(HPI_calculated, aes(x = SITE, y = log_HPI, fill = factor(YEAR))) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(x = "Site", 
       y = "Log Habitat Productivity Index", 
       fill = "Year") +
  scale_fill_manual(values = my_palette) +  # Use the colorblind-friendly palette
  theme_minimal() +
  ...
  coord_cartesian(clip = "off");hpi.plot
...
```{r}
# Combine the plots with patchwork for all years of kelp and rate of change
production.hpi <- production_plot | hpi.plot +
  plot_annotation(tag_levels = 'A') + 
  theme(plot.tag = element_text(size = 20, face = "bold"));production.hpi

# Save the combined plot
ggsave("output/figures/production.HPI.png", production.hpi, width = 20, height = 10, units = "in", dpi = 300)
