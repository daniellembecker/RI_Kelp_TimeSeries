...
kelp.timeseries.2016 <- ggplot(aggregated.kelp.morph, aes(x = YEAR, y = mean_kelp_biomass, shape = SITE)) +
  ...
  labs(x = "Year", y = expression("Mean Kelp Biomass (g/" ~ m^2 ~ ")"), shape = "Site") +
  theme_bw() +
...
rate_of_change <- ggplot(slope_results, aes(x = rate_of_change_per_year, y = mean_nonlog_avg_kelp_biomass, shape = SITE)) +
  ...
  labs(
    ...
    y = expression("Mean Kelp Biomass (g/"~m^2~")"), shape = "Site") +
  theme_bw() +
  ...
  theme(
    ...
    legend.text = element_text(size = 16),
    legend.title = element_text(size = 18), ,
    legend.position = "none");rate_of_change
...
# Plot two plots together using patchwork for production and HPI
```{r}
# Combine the plots with patchwork for all years of kelp and rate of change
