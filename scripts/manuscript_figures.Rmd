...
PB$biomass <- with(PB, coef.a * (actual_size)^coef.b)
...
nmds_plot <- ggplot(scores, aes(x = NMDS1, y = NMDS2, color = SITE, shape = as.factor(YEAR))) +
  ...
  stat_ellipse(aes(group = YEAR), level = 0.95, linetype = 2, size = 1, alpha = 0.2)+
  ...
  theme(
    ...
    axis.text.y = element_text(size = 18),
    legend.text = element_text(size = 16),
    legend.title = element_text(size = 18));nmds_plot
...
environmental_data <- environmental_data[non_zero_rows, ]
...
permanova_df <- data.frame(
  ...
  Df = c(1, 4, 37, 42),
  SumOfSqs = c(0.2632, 2.8619, 7.4000, 10.5251),
  R2 = c(0.02501, 0.27191, 0.70308, 1.00000),
  F = c(1.3160, 3.5774, NA, NA),
  Pr_F = c(0.235, 0.001, NA, NA)  # Changed column name to Pr_F
)
...
flex_perm <- flex_perm %>%
  ...
  bold(i = ~ Factor == "YEAR", j = "Pr_F") %>%  # Changed to match the new column name
...
# For the SIMPER analysis, we'll use the original (non-transformed) data, but still excluding the all-zero rows
...
simper_data <- data.frame(
  Species = simper_result$`Fort Wetherill_King's Beach`$species,
  Contribution = simper_result$`Fort Wetherill_King's Beach`$average,
  Fort_Wetherill = simper_result$`Fort Wetherill_King's Beach`$ava,
  Kings_Beach = simper_result$`Fort Wetherill_King's Beach`$avb
)
...
# Create the plot
ggplot(simper_long, aes(x = Species, y = Abundance, fill = Site)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(data = simper_data, 
            aes(x = Species, y = max(simper_long$Abundance) + 0.5, 
...
SITE      1   0.2632 0.02501 1.3160  0.235    
YEAR      4   2.8619 0.27191 3.5774  0.001 ***
Residual 37   7.4000 0.70308                  
Total    42  10.5251 1.00000  


SIMPER results:
Contrast: Fort Wetherill_King's Beach 
            average      sd   ratio     ava     avb cumsum     p  
molluscs    0.56710 0.30146 1.88120 7.64700 5.84300  0.894 0.803  
echinoderms 0.04820 0.10148 0.47480 0.14700 0.34300  0.970 0.067 .
crustacean  0.01920 0.04652 0.41350 0.30000 0.03700  1.000 0.713  
---
Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
Permutation: free
Number of permutations: 999
