...
Invertebrate community structure calculations
```{r}
...
Figure and calculations for functional group change in relative abundance over time using invert count data
```{r}
# Ensure only numeric columns are selected for vegdist
...
invert.abun$log_rel_abundance <- log(invert.abun$abundance + 1.1)

# Check for normality of transformed data
anova_model <- aov(log_rel_abundance ~ type * YEAR, data = invert.abun)
...
interaction_data <- invert.abun %>%
  mutate(interaction = interaction(type, YEAR))

kruskal_test_interaction <- kruskal.test(log_rel_abundance ~ interaction, data = interaction_data)
...
data:  log_rel_abundance by interaction
