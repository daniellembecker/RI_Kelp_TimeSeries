...
#combine with fish aggregated data
fish_data_agg <- rbind.fill(fish_data, fish_Aggregated)

# remove columns with X fromm numerical order of columns
fish_data_agg <- fish_data_agg[ -c(16) ]

# Rename ft. wetherill to fort wetherill
fish_data_agg$SITE[fish_data_agg$SITE == "Kings Beach"] <- "King's Beach"

# Rename fort weatherill
fish_data_agg$SITE[fish_data_agg$SITE == "Fort Weatherill"] <- "Fort Wetherill"

# Rename transcets to not have FW or KB before them
fish_data_agg$TRANSECT <- gsub("FW\\s*", "", fish_data_agg$TRANSECT)
fish_data_agg$TRANSECT <- gsub("KB\\s*", "", fish_data_agg$TRANSECT)

# output updated dataframe
write.csv(fish_data_agg, "output/cleaned_data/fish.data.clean.csv", row.names = FALSE)
...
# Melt only columns AGH:WHEL into long format
QUAD_long <- QUAD %>%
  pivot_longer(cols = AGH:Oar.Weed.Total, names_to = "SP_CODE", values_to = "COUNT")
...
# use rbind.fill() method in R to combine data frames with different columns
rug_data <- rbind.fill(rugosity_2019_DEM, rugosity_2020_DEM, rugosity_2021_DEM)

#remove 22-25 columns for observers not needed information
rug_data <- rug_data[ -c(22:25) ]
