...
aggregated_data <- merged.dat %>%
  ...
  summarize(mean_fish_biomass = mean(fish_biomass, na.rm = TRUE),
            mean_kelp_cover = mean(kelp_cover, na.rm = TRUE)) %>%
  ungroup()
...
# Create scatter plot
scatter_plot <- ggplot(aggregated_data, aes(x = mean_kelp_cover, y = mean_fish_biomass)) +
  geom_point() +
  geom_smooth(method = "lm", se = TRUE, formula = y ~ x, color = "blue") +
  geom_text(data = lm_summary, aes(label = paste("R² =", round(r.squared, 2), ", p =", signif(p.value, 2))),
            x = Inf, y = Inf, hjust = 1, vjust = 1, size = 4, parse = TRUE) +
  labs(x = "Kelp cover (%)", y = "Fish biomass (g/m2)",
       title = "Fish Biomass vs Kelp Cover",
       caption = "Linear regression with 95% CI bands") +
  theme_minimal()
