...
production_plot <- ggplot(pb_summary, aes(x = YEAR, y = mean_production, color = TREATMENT, group = TREATMENT)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 18),  # Rotate x-axis labels if needed
        ...
        legend.text = element_text(size = 16),
        legend.title = element_text(size = 18)) +
  scale_y_continuous(limits = c(0, 8), expand = c(0, 0));production_plot # Additional attempt to change legend title  # Use viridis colorblind-friendly palette
...
production_diff_plot <- ggplot(merged_data, aes(x = YEAR, y = production_difference, group = TREATMENT_kelp)) +
  geom_point(size = 2) +  # Points with light blue outline
  geom_line() +  # Lines with light blue color
